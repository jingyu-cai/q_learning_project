...
- During testing, we found that the dumbbell could be knocked to the ground when the robot picked it up or dropped it down. To tackle this, we open the gripper wider when it's about to lift the dumbbell, and also when the robot release the dumbbell at the correct block, we let it sleep for 0.5s, and then step back with a slower speed. 
